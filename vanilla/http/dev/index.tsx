...
!(async ()=>{
  ...
  let host = pkg_json.config.host
  let STDB = await client(host, name, false)
  ...
  let sqlQuery = await STDB.database.sql('SELECT username FROM UserTable')
  console.log('SQL Res: ', sqlQuery[0] ? sqlQuery[0].rows.map(v=>v[0]['0']) : sqlQuery)
  ...
  let schemaEntity = await STDB.database.schema('table', 'UserTable')
